...
echo ""
echo -e "  ${BOLD}Ссылки для клиента:${NC}"
echo -e "    ${CYAN}vless://${UUID1}@${SERVER_IP}:443?encryption=none&security=reality&sni=www.microsoft.com&fp=chrome&pbk=${PUBLIC_KEY}&sid=${SHORT_ID1}&type=xhttp&path=%2Fapi%2Fv1%2Fdata&mode=auto#user1${NC}"
echo -e "    ${CYAN}vless://${UUID2}@${SERVER_IP}:443?encryption=none&security=reality&sni=www.microsoft.com&fp=chrome&pbk=${PUBLIC_KEY}&sid=${SHORT_ID2}&type=xhttp&path=%2Fapi%2Fv1%2Fdata&mode=auto#user2${NC}"
echo ""
if command -v telemt &>/dev/null; then
